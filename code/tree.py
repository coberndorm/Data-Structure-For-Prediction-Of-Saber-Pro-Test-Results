...
def tree_maker(node, checked_columns, height, students, list_students_left, list_students_right):
    ...
    if node.gini_left <= node.left.gini or height >= 6:
    ...
    if node.gini_right <= node.right.gini or height >= 6:
...
def runClassify(datos_train, datos_test):
    ...
    for id in list_students:
        if classify(id, root, test_students) == test_students[id].exito:
            prediction += 1
    return prediction / len(list_students)


if __name__ == '__main__':
    print('\n')
    print (runClassify('../datos/datos_train0.csv', '../datos/datos_test4.csv'))
